...
2. Отсутствие стандартной возможности запросить переданные шаблону
   ...
   аттрибут `__orig_class__`, но он не регламентирован и не доступен
...
* Задание одинаковых значений приводит не к созданию новой сущности, а к
  ...
  `TemplateClassA[int, float] == TemplateClassB[int, float]`
  возвращает `True`, а
  `TemplateClassA[int, float] == TemplateClassB[int, str]`
  вернет `False`.
...
- [ ] Реализовать разрешение `EvGen` без наследования.
